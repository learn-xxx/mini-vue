...
var object = {
  ...
  toPairsIn, transform: transform$1, unset, update, updateWith,
  values, valuesIn
};
...
const printStage2$1 = once(printStage);
const printStage3$1 = once(printStage);
const printSentence1$2 = once(printSentence);
const printSentence2$1 = once(printSentence);
function trackEffects(dep) {
    printStage2$1('【reactive】收集依赖开始');
    printSentence1$2('当前依赖：', cloneDeep(activeEffect));
    dep.add(activeEffect);
    activeEffect.deps.push(dep);
    printSentence2$1('依赖树(target -> key -> dep):', targetMap);
    printStage3$1('【reactive】收集依赖结束');
}
function isTracking() {
    //是否开启收集依赖 & 是否有依赖
...
const printStage5$1 = once(printStage);
const printStage6$1 = once(printStage);
function triggerEffects(dep) {
    for (const effect of dep) {
        printStage5$1('【reactive】触发依赖开始，开始更新数据');
        if (effect.scheduler) {
            effect.scheduler();
        }
        else {
        ...
        printStage6$1('【reactive】触发依赖完成，数据已更新');
    }
}
//创建一个依赖
...
const printSentence4$1 = once(printSentence);
class RefImpl {
    constructor(value) {
        this._v_isRef = true;
        //判断value是不是Object
    ...
    set value(newValue) {
        if (hasChanged(newValue, this._rawValue)) {
            printSentence4$1('【响应式数据发生了改变', '新值为', newValue, '旧值为', this._rawValue, '】');
            // 一定先修改value
...
const printSentence1$1 = once(printSentence);
function ref(value) {
    const ref = new RefImpl(value);
    printSentence1$1('创建一个ref响应式对象：', ref);
    return ref;
}
...
const printTip1$1 = once(printTip);
const printStage1$1 = once(printStage);
const printStage2 = once(printStage);
const printSentence1 = once(printSentence);
function setupStatefulComponent(instance) {
    const Component = instance.type;
    //创建代理
    printTip1$1('代理对象proxy是属于组件实例的，通过劫持get操作，判断数据是来自props、setupState等，返回数据的结果');
    ...
    if (setup) {
        ...
        printStage1$1('【runtime】开始执行组件setup函数');
...
function printTable(prefix, ...args) {
    if (!checkStatus()) {
        return;
    }
...
const instanceToTable = (instance) => ({
    vnode: {
...
export { createApp, createVNode as createElementVNode, createRenderer, createTextNode, getCurrentInstance, h, inject, nextTick, offLearn, onLearn, provide, reactive, ref, registerRuntimeCompiler, renderSlots, toDisplayString };
